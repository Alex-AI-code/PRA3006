...
(async() => {
    ...
    //update(Breast_cancer)   ------> print the pie chart
    ...
    //console.log(data_pc)
    //console.log(Array_of_transformed_units_pc)

    const chart_data_bc = []
    const chart_data_pc = []

    function couple_data_with_transformed_units(data, transformed_units, chart_data) {
        // iterate through data, if unit is major one add name and dose to new dic else
        // add name and add new dose
        // wahtever is not equal to winning, else if gram assign new value if micro assign new value and add to dic
        var j = 0;
        var z = 0;

        for (let i = 0; i < data.length; i++) {
            // if json_object is equal to breast cancer append
            if (data[i]['unitsLabel'] == 'milligram') {
                chart_data.push({ drug: data[i]['mL'], count: data[i]['dose'] })
            } else if (data[i]['unitsLabel'] == 'gram') {
                chart_data.push({ drug: data[i]['mL'], count: transformed_units[0][j] })
                j++;
            } else {
                chart_data.push({ drug: data[i]['mL'], count: transformed_units[1][z] })
                z++;
            }
        }
    }
    ...
    couple_data_with_transformed_units(data_bc, Array_of_transformed_units_bc, chart_data_bc)
    couple_data_with_transformed_units(data_pc, Array_of_transformed_units_pc, chart_data_pc)
        //console.log(chart_data_bc)
        //console.log(chart_data_pc)

    // works like a charm

    // try on bar graph
    update_bar(chart_data_bc) // ------> print the bar chart
